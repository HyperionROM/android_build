...
def write_to_manifest(manifest):
    ...
    with open('/'.join([local_manifest_dir, "roomservice.xml"]), 'w') as f:
        f.write(raw_xml)
    print("Device files written to local manifest")
...
def parse_device_from_folder(device):
    ...
    if len(search) > 1:
        ...
        print("Your device was not found. Attempting to retrieve device repository from HyperionROMs Github..")
        location = parse_device_from_manifest(device)
    return location


def parse_dependency_file(location):
    dep_file = "hyperion.dependencies"
