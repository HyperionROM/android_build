...
function check_product()
{
    ...
    if (echo -n $1 | grep -q -e "^hyperion_") ; then
       HYPERION_BUILD=$(echo -n $1 | sed -e 's/^hyperion_//g')
       export BUILD_NUMBER=$((date +%s%N ; echo $HYPERION_BUILD; hostname) | openssl sha1 | sed -e 's/.*=//g; s/ //g' | cut -c1-10)
    else
       HYPERION_BUILD=
    fi
    export HYPERION_BUILD
...
function print_lunch_menu()
{
    ...
    echo
    if [ "z${HYPERION_DEVICES_ONLY}" != "z" ]; then
...
function breakfast()
{
    ...
    if [ $# -eq 0 ]; then
        ...
        if [ $? -eq 0 ]; then
            ...
            lunch hyperion_$target-userdebug
        fi
    fi
    return $?
}
...
{
    if [ "$OUT" ] ; then
        MODVERSION=$(get_build_var HYPERION_VERSION)
    ...
    if (adb shell cat /system/build.prop | grep -q "ro.cm.device=$HYPERION_BUILD");
    then
...
EOF
    ...
    return $?
        ...
        echo "The connected device does not appear to be $HYPERION_BUILD, run away!"
    fi
}
...
function fixup_common_out_dir() {
    ...
    if [ ! -z $HYPERION_FIXUP_COMMON_OUT ]; then
